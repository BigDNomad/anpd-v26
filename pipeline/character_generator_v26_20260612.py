...
def load_series_artifacts(series_dir):
    ...
    raw_profiles = load_json(profiles_path) if profiles_path else {}

    # Normalize: series-level file may use {"characters": [list of dicts]}
    # envelope (Schema v1.1.0 §3.4 alternate representation).
    # Downstream expects {name: profile_dict}.
    if (isinstance(raw_profiles, dict)
            and 'characters' in raw_profiles
            and isinstance(raw_profiles['characters'], list)):
        series_profiles = {
            p['name']: p for p in raw_profiles['characters']
            if isinstance(p, dict) and 'name' in p
        }
    else:
        series_profiles = raw_profiles
