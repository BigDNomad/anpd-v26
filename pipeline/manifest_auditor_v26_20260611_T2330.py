...
def _resolve_shim_to_canonical(file_path: str) -> str:
    """If file_path is a V26 shim, return the canonical file path it redirects to.

    Detects the pattern:
        _canonical = _il.import_module("pipeline.<canonical_name>")
    Returns the resolved file path, or the original if not a shim.
    """
    import re
    try:
    ...
    # Match: import_module("pipeline.<canonical>")  or  import_module("<canonical>")
    match = re.search(
        r'import_module\(\s*["\'](?:pipeline\.)?([a-zA-Z_0-9]+)["\']\s*\)',
        source,
    )
    if match:
        canonical_name = match.group(1)
        canonical_path = os.path.join(os.path.dirname(file_path), canonical_name + ".py")
...
def _parse_components_dict_keys(master_controller_path: str) -> set | None:
    ...
    If the file is a shim, follows the redirect to the canonical module.
    Returns the set of string keys, or None if not found.
    """
    resolved = _resolve_shim_to_canonical(master_controller_path)
    try:
