...
# V26: COMPONENTS values are canonical module names for ``python3 -m``
# invocation, no longer file paths.  Dict keys are stable component
# identifiers.
COMPONENTS: dict[str, str] = {
    "preflight":                 "pipeline.preflight_v26_20260611",
    "synopsis_generator":        "pipeline.synopsis_generator_v26_20260611",
    "synopsis_auditor":          "pipeline.synopsis_auditor_v26_20260611_T0500",
    ...
    "scene_writer":              "pipeline.scene_writer_v26_20260611",
    "scene_auditor":             "pipeline.scene_auditor_v26_20260611",
    "state_tracker":             "pipeline.state_tracker_v26_20260611",
    "scene_formatter":           "pipeline.scene_formatter_v26_20260611",
    "manuscript_auditor":        "pipeline.manuscript_auditor_v25_v26_20260611",
    "manuscript_assembler":      "pipeline.manuscript_assembler_v26_20260611",
    "outline_comparator":        "pipeline.outline_comparator_v26_20260611",
    "manuscript_summarizer":     "pipeline.manuscript_summarizer_v26_20260611",
    "manifest_auditor":          "pipeline.manifest_auditor_v26_20260611",
    "formatter":                 "pipeline.formatter_v26_20260611",
    "book_archive":              "pipeline.book_archive_v26_20260611",
    "anpd_export":               "pipeline.anpd_export_v26_20260611",
}
...
def run_component_subprocess(
    ...
    module_name = COMPONENTS[component_name]
    started_at = datetime.now(timezone.utc).isoformat()
    started_at_epoch = datetime.now(timezone.utc).timestamp()

    # Stub case: component module cannot be found.
    try:
        import importlib
        ...
        return {
            "exit_code": -1,
            "stdout": "",
            "stderr": f"component {component_name!r} not importable as {module_name}",
    ...
    # Real invocation via python3 -m <module_name>.
...
# ─── RuntimeVerifier startup gate ─────────────────────────────────────────────

def _verify_startup(verifier: Any) -> list[dict]:
    """Verify all manifest-registered components are importable at startup.

    Returns a list of finding dicts (empty = pass). Each finding has
    rule_id, severity, component_name, message.
    """
    import importlib
    findings = []
    for name, entry in verifier._component_entries.items():
        module_name = COMPONENTS.get(name)
        if not module_name:
            continue
        try:
            importlib.import_module(module_name)
            ...
            findings.append({
                ...
                "severity": "A",
                "component_name": name,
                "message": f"Component {name!r} module {module_name!r} "
...
def run_pipeline(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if manifest_path.exists():
            ...
            # ── Startup gate: verify all registered components are importable ──
            startup_failures = _verify_startup(verifier)
            if startup_failures:
                print(f"\n  [runtime_verifier] STARTUP GATE FAILED — {len(startup_failures)} issue(s)")
                for sf in startup_failures:
                    print(f"    {sf['rule_id']}: {sf['message']}")
                # Write STOP_REPORT and halt.
                stop_path = stop_report_path(args.book_dir)
                os.makedirs(os.path.dirname(stop_path), exist_ok=True)
                with open(stop_path, "w", encoding="utf-8") as fh:
                    json.dump({
                        "source": "runtime_verifier_startup",
                        "findings": startup_failures,
                        "pipeline_state_snapshot": {
                            k: v for k, v in pipeline_state.items()
                            if k not in ("_args", "_started_at_epoch")
                        },
                    }, fh, indent=2)
                print(f"\n  STOP_REPORT written: {stop_path}")
                _finalize_receipt(pipeline_state, args.book_dir)
                return 1
            print("  [runtime_verifier] startup gate passed — all components importable")
        else:
