...
def handle_scene_loop(
    ...
    For each scene in synopsis (bounded by --start-scene / --end-scene):
    ...
    try:
        from pipeline.synopsis_parser import parse_synopsis
    except ImportError:
        from synopsis_parser import parse_synopsis

    try:
        synopsis_structure = parse_synopsis(synopsis_path)
    except (ValueError, OSError) as exc:
        return _wrap_class_a(
            "scene_loop",
            f"synopsis parse failed: {exc}",
            phase=5,
        )

    # Build scene_map dict from synopsis (same shape downstream code expects).
    scene_map: dict[int, dict] = {}
    for scene in synopsis_structure.all_scenes:
        scene_map[scene.scene_number] = {
            "title": scene.title,
            "body": scene.body,
        }
    ...
    if not scene_map:
        return _wrap_class_a(
            "scene_loop",
            "synopsis produced zero scenes",
            phase=5,
        )

    # ── Contiguity gate: scene numbers MUST be 1..N with zero gaps ──
    # Completeness is judged against the synopsis's own declared
    # sequence, never against "what I happened to parse."
    ...
    if missing:
        return _wrap_class_a(
            "scene_loop",
            f"synopsis scene numbers are not contiguous 1..{available[-1]}: "
...
def handle_chapter_assembly(args, pipeline_state, effective_config) -> dict:
    ...
    result = mc.run_component_subprocess(
        ...
        [
            ...
            "--synopsis", os.path.join(args.book_dir, "work", "synopsis.md"),
    ...
    # ── manuscript_assembler: concatenate chapters → manuscript.md ──
    # Per manifest: scenes → chapters → ASSEMBLER → Gate 3 audits manuscript.md.
    output_dir = os.path.join(args.book_dir, "out")
    asm_result = mc.run_component_subprocess(
        "manuscript_assembler",
        [
            "--chapters-dir", chapters_dir,
    ...
    if asm_result["stubbed"]:
        return _handle_stubbed("manuscript_assembler", None, pipeline_state)

    if asm_result["stop_report_written_during_call"]:
        return {"verdict": "halt", "findings": [], "via": "component_stop_report"}

    if asm_result["exit_code"] != 0:
        return _wrap_class_a(
            "manuscript_assembler",
            f"exit code {asm_result['exit_code']}: {asm_result['stderr'][:200]}",
            phase=6,
        )
    ...
    if not os.path.isfile(manuscript_path):
        return _wrap_class_a(
            "manuscript_assembler",
            f"subprocess returned 0 but no manuscript.md at {manuscript_path}",
            phase=6,
        )
...
def handle_manuscript_gate(
    ...
    1. Invoke manuscript_auditor_v25 (V26: replaces V24 stub)
    ...
    """
    # Prefer scene-level granularity (out/scenes/ = 100 individual scene files).
    # Fall back to chapter-level (out/chapters/) only if scenes dir absent.
    scenes_dir = os.path.join(args.book_dir, "out", "scenes")
    chapters_dir = os.path.join(args.book_dir, "out", "chapters")
    if os.path.isdir(scenes_dir) and os.listdir(scenes_dir):
        manuscript_dir = scenes_dir
    else:
        manuscript_dir = chapters_dir

    retries = args.max_retries_per_gate

    # Build CLI args for manuscript_auditor_v25.
    auditor_args = ["--manuscript-dir", manuscript_dir]

    # Synopsis must be passed explicitly — the auditor's _resolve_synopsis
    # cannot find work/synopsis.md from the out/ subtree.
    synopsis_path = os.path.join(args.book_dir, "work", "synopsis.md")
    if os.path.isfile(synopsis_path):
        auditor_args += ["--synopsis", synopsis_path]

    if hasattr(args, "series_bible") and args.series_bible:
    ...
    audit_output_dir = os.path.join(args.book_dir, "out", "audit")
    auditor_args += ["--output-dir", audit_output_dir]
