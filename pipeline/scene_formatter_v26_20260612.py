...
- synopsis.md from {book_dir}/work/ (scene_map RETIRED per operator ruling
  2026-05-06; synopsis carries chapter and scene structure directly)
...
1. Explicit assignment via synopsis chapter headings.
   The synopsis carries `## Chapter K — Title` headings with scene entries
   under each chapter. Each scene inherits its chapter from the synopsis
   structure.

2. Computed assignment via D009-validated scenes-per-chapter ratio.
   When synopsis has no chapter headings, scenes are divided evenly
   into target_chapter_count chapters. Per preflight D009 the ratio is
   constrained to 3, 4, or 5.

Mode 1 takes precedence when chapter headings are present in the synopsis.
...
# ─── Synopsis parsing (replaces scene_map) ───────────────────────────────────

def _import_synopsis_parser():
    """Import synopsis_parser with package/bare fallback."""
    try:
        from pipeline.synopsis_parser import parse_synopsis
    except ImportError:
        from synopsis_parser import parse_synopsis
    return parse_synopsis


def parse_synopsis_for_chapters(synopsis_path: str) -> tuple[list[dict], list[dict]]:
    """Parse synopsis.md into (scenes, chapters) for chapter assignment.

    Returns the same shape as the retired parse_scene_map so that
    assign_scenes_to_chapters_explicit() works unchanged.

    `position` values are synthetic ordinals that preserve the
    chapter-precedes-its-scenes invariant.
    """
    if not os.path.isfile(synopsis_path):
        raise FileNotFoundError(f"synopsis not found at {synopsis_path}")

    parse_synopsis = _import_synopsis_parser()
...
def run_scene_formatter(
    ...
    print(f"  synopsis:             {synopsis_path}")
    ...
    # Parse synopsis for chapter structure
    try:
        map_scenes, map_chapters = parse_synopsis_for_chapters(synopsis_path)
    except (OSError, FileNotFoundError) as exc:
        msg = f"synopsis parse failed: {exc}"
        write_stop_report(
            book_dir,
            error_message=msg,
            suggested_fix="ensure synopsis.md exists and is readable",
            file_path=synopsis_path,
        )
        print(f"  FAIL: {msg}")
        return (1, {"chapters_written": 0, "missing_scenes": [], "mode": "n/a"})
    ...
    if map_chapters:
        ...
        if unassigned:
            msg = (
                ...
                f"to any chapter heading in synopsis (unassigned: "
            ...
            write_stop_report(
                ...
                error_message=msg,
                suggested_fix=(
                    "either add Chapter N headings to synopsis covering all scenes, "
