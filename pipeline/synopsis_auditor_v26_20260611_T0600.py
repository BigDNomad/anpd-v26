...
[Q19] Post-climax scene classification: exactly 2 DENOUEMENT scenes required.
...
- Exactly 2 DENOUEMENT scenes are required.
- FAIL if DENOUEMENT count is not exactly 2.
...
def _count_denouement_scenes(q19_item: dict) -> tuple[int, str]:
    ...
    Falls back to legacy resolution_scenes count if no post_climax_scenes data.
    """
    scenes = q19_item.get('post_climax_scenes', [])
    if not scenes:
        # Fallback: parse from note text if LLM didn't produce structured data
        note = q19_item.get('note', '')
        # Count "DENOUEMENT" occurrences in note as rough fallback
        denouement_count = note.upper().count('DENOUEMENT')
        return (denouement_count, f"(parsed from note text, {denouement_count} DENOUEMENT mentions)")
...
def consolidate(call_1_data, call_2_data, title, word_count, effective_config):
    ...
    # AFTERMATH scenes are unlimited; exactly 2 DENOUEMENT required.
    for item in all_items:
        if item.get('id') == 'Q19':
            ...
            if denouement_count == 2:
                ...
                item['note'] = (
                    ...
                    f"(exactly 2 required). {classification_table}"
                )
            else:
                item['verdict'] = 'FAIL'
                item['note'] = (
                    f"Post-climax classification: {denouement_count} DENOUEMENT scenes "
                    f"(exactly 2 required). FAIL: {'excess' if denouement_count > 2 else 'deficit'}. "
...
def main():
    parser = argparse.ArgumentParser(description='ANPD V24 Synopsis Auditor — Gate 1')
    ...
    parser.add_argument('--series-config', required=True, help='Path to series_config.json (drives Haiku rubric thresholds and model identifier)')
    args = parser.parse_args()

    # Load effective config — required per §2.1
    try:
    ...
    print(f"\n{'='*70}")
    print(f"  ANPD V23 — SYNOPSIS AUDIT (2 SEQUENTIAL CALLS)")
    print(f"{'='*70}")
    ...
    # Load character profiles from series dir
    ...
    # Derive book_dir from synopsis path for output
    ...
    # ── Deterministic synopsis integrity checks ──
    parsed_scenes = parse_synopsis(synopsis)
    deterministic_findings = check_synopsis_integrity(
        parsed_scenes, intake, args.synopsis
    )
    if deterministic_findings:
        print(f"  Deterministic checks: {len(deterministic_findings)} finding(s)")
    else:
        print(f"  Deterministic checks: all clear ({len(parsed_scenes)} scenes parsed)")
    ...
    banned_content_count = len(banned_validation_findings)
    if banned_data is not None and parsed_scenes:
        name_findings = check_banned_names(parsed_scenes, banned_data, args.synopsis)
        phrase_findings = check_banned_phrases(parsed_scenes, banned_data, args.synopsis)
        deterministic_findings.extend(name_findings)
        deterministic_findings.extend(phrase_findings)
        banned_content_count += len(name_findings) + len(phrase_findings)

    if banned_content_count:
        print(f"  Banned content checks: {banned_content_count} finding(s)")
    else:
        print(f"  Banned content checks: all clear")
    ...
    if chapter_findings:
        print(f"  Chapter count check: {len(chapter_findings)} finding(s)")
    else:
        print(f"  Chapter count check: all clear")

    # ── Call 1: Structural and Mechanical Checks ──
    print(f"\n  Call 1/2: Structural and Mechanical Checks...")
    ...
    try:
        ...
        call_1_data['_elapsed_seconds'] = round(elapsed_1, 1)

        verdicts_1 = [i.get('verdict', '?') for i in call_1_data.get('items', [])]
        fail_c = verdicts_1.count('FAIL')
        weak_c = verdicts_1.count('WEAK')
        pass_c = verdicts_1.count('PASS') + verdicts_1.count('N/A')
        print(f"  Call 1 complete — {elapsed_1:.0f}s — "
              f"PASS:{pass_c} WEAK:{weak_c} FAIL:{fail_c}")
        print(f"    Scenes: {call_1_data.get('total_scenes', '?')} total, "
              f"{call_1_data.get('action_scenes', '?')} action "
              f"({call_1_data.get('action_scene_percentage', '?')}%), "
              f"{call_1_data.get('resolution_scenes', '?')} resolution")

    except json.JSONDecodeError as e1:
        print(f"  Call 1 JSON parse failed (attempt 1): {e1}", file=sys.stderr)
        print(f"  Retrying with stricter JSON instruction...")
        retry_prompt = call_1_prompt + "\n\nReturn only valid JSON. No preamble, no code fences, no commentary. If you cannot produce valid JSON, return the literal string PARSE_ERROR."
        try:
            ...
            print(f"  Call 1 retry succeeded")
        except (json.JSONDecodeError, Exception) as e2:
            raw_preview = (raw_1 or '')[:500]
            error_msg = (f"JSON parse failed after retry in synopsis_auditor Call 1 (Structural and Mechanical Checks). "
                        f"Raw model output (first 500 chars): {raw_preview}")
            ...
            stop_report = {
                ...
                "suggested_fix": "Re-run with increased max_tokens, or inspect model output for non-JSON preamble. If this repeats, the pass prompt needs revision.",
                "pipeline_state": "Call 1 failed; Call 2 not attempted"
            }
        ...
        raw_preview = (raw_1 or '')[:500]
        error_msg = f"API error in synopsis_auditor Call 1: {e}. Raw output: {raw_preview}"
        print(f"  FATAL: {error_msg}", file=sys.stderr)
        reports_dir = os.path.join(book_dir, 'out', 'reports')
        os.makedirs(reports_dir, exist_ok=True)
        stop_report = {
    ...
    # ── Call 2: Quality and Engagement Checks ──
    print(f"\n  Call 2/2: Quality and Engagement Checks...")
    ...
    try:
        ...
        call_2_data['_elapsed_seconds'] = round(elapsed_2, 1)

        verdicts_2 = [i.get('verdict', '?') for i in call_2_data.get('items', [])]
        fail_c = verdicts_2.count('FAIL')
        weak_c = verdicts_2.count('WEAK')
        pass_c = verdicts_2.count('PASS') + verdicts_2.count('N/A')
        print(f"  Call 2 complete — {elapsed_2:.0f}s — "
              f"PASS:{pass_c} WEAK:{weak_c} FAIL:{fail_c}")

    except json.JSONDecodeError as e1:
        print(f"  Call 2 JSON parse failed (attempt 1): {e1}", file=sys.stderr)
        print(f"  Retrying with stricter JSON instruction...")
        ...
        try:
            ...
            print(f"  Call 2 retry succeeded")
        except (json.JSONDecodeError, Exception) as e2:
            raw_preview = (raw_2 or '')[:500]
            error_msg = (f"JSON parse failed after retry in synopsis_auditor Call 2 (Quality and Engagement Checks). "
                        f"Raw model output (first 500 chars): {raw_preview}")
            print(f"  FATAL: {error_msg}", file=sys.stderr)
            reports_dir = os.path.join(book_dir, 'out', 'reports')
            os.makedirs(reports_dir, exist_ok=True)
            stop_report = {
                ...
                "file_path": os.path.abspath(__file__),
                "suggested_fix": "Re-run with increased max_tokens, or inspect model output for non-JSON preamble. If this repeats, the pass prompt needs revision.",
                "pipeline_state": "Call 1 completed successfully; Call 2 failed"
            }
            stop_path = os.path.join(reports_dir, 'STOP_REPORT.json')
            with open(stop_path, 'w') as f:
                json.dump(stop_report, f, indent=2)
        ...
        raw_preview = (raw_2 or '')[:500]
        error_msg = f"API error in synopsis_auditor Call 2: {e}. Raw output: {raw_preview}"
        print(f"  FATAL: {error_msg}", file=sys.stderr)
        reports_dir = os.path.join(book_dir, 'out', 'reports')
        os.makedirs(reports_dir, exist_ok=True)
        stop_report = {
    ...
    # Q9 deterministic override: suspense counts as action; Haiku miscounts, recount from [TYPE:] tags.
    ...
    # V24 finding schema output (White Paper §3.8).
    # Runs alongside the legacy report formats during the Phase 2 transition.
    try:
        findings_list = deterministic_findings + items_to_findings(call_1_data, call_2_data, args.synopsis, effective_config)
        ...
        stop_report = {
            ...
            "error_message": error_msg,
            "file_path": os.path.abspath(__file__),
            "suggested_fix": "Finding conversion encountered invalid Haiku output. See error_message for the specific cause. If a rubric ID is missing from RUBRIC_METADATA, add the mapping. If a verdict is outside VALID_VERDICTS, either Haiku returned malformed output (re-run, inspect raw response) or a new verdict was introduced and VALID_VERDICTS needs updating.",
            "pipeline_state": "Call 1 and Call 2 completed; consolidate completed; finding conversion failed"
        }
        stop_path = os.path.join(reports_dir, 'STOP_REPORT.json')
        with open(stop_path, 'w') as f:
            json.dump(stop_report, f, indent=2)
    ...
    print(f"  (Filenames use 'synopsis_audit_report' to avoid glob collision with synopsis files)")
...
def audit_synopsis(
    ...
    cmd = [
        sys.executable, "-m", "pipeline.synopsis_auditor_v26_20260611_T0600",
