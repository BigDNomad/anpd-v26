...
def _merge_multipass_results(
    ...
    1. Purely LLM-judged (Q1, Q2, Q5, Q10, Q12, Q13, Q16, Q17, Q18):
       majority-vote verdicts across passes.
    2. LLM-judged with deterministic post-processing (Q8, Q19):
       majority-aggregate RAW MATERIAL only (Q19: per-scene classifications;
       Q8: word count is already deterministic) → apply bands → band verdict
       is FINAL, no LLM verdict can override it.
    ...
    for items_list in [final_c1.get('items', []), final_c2.get('items', [])]:
        for item in items_list:
            qid = item.get('id')
            if qid and qid in q_verdicts and qid not in DETERMINISTIC_CHECKS:
