...
def consolidate(call_1_data, call_2_data, title, word_count, effective_config):
    ...
    Q8/Q19 deterministic overrides are applied upstream in
    _merge_multipass_results (or per-pass in _run_single_audit_pass).
    This function computes the overall verdict from final per-check verdicts.
    """
    all_items = []
...
# Extracted so the same logic runs per-pass (for accurate logging),
# after merge (for the final verdict), and in tests.

Q8_HARD_FLOOR = 13000


def _q8_band_verdict(word_count: int, word_min: int, word_max: int) -> str:
    """Deterministic Q8 verdict from word-count bands."""
...
def _q8_band_note(word_count: int, word_min: int, word_max: int) -> str:
    """Deterministic Q8 note from word-count bands."""
    if word_count < Q8_HARD_FLOOR:
        return (
        ...
        return f"Synopsis word count is {word_count:,} words. In target range ({word_min:,}\u2013{word_max:,})."
    else:
        return (
            f"Synopsis word count is {word_count:,} words. "
            f"Above maximum {word_max:,}. FAIL: over-padded."
        )


def _q19_band_verdict(denouement_count: int) -> str:
    """Deterministic Q19 verdict from denouement-count bands."""
...
def _q19_band_note(denouement_count: int, classification_table: str) -> str:
    """Deterministic Q19 note from denouement-count bands."""
    if denouement_count == 2:
        return (
...
# ── Deterministic Q2 (pillar markers) ────────────────────────────────────────
# When [PILLAR: TWIST1/TWIST2/TWIST3] markers are present in scene headers,
# Q2 is computed deterministically from scene positions. When markers are
# absent, the legacy 3-pass LLM path is unchanged.

_TWIST_PILLARS = ('TWIST1', 'TWIST2', 'TWIST3')
_Q2_PASS_BAND = 7    # ±7% → PASS
_Q2_WEAK_BAND = 12   # ±12% → WEAK; beyond → FAIL


def _q2_has_pillar_markers(scenes: List[Scene]) -> bool:
    """Return True if any scene carries a TWIST pillar marker."""
    return any(s.pillar in _TWIST_PILLARS for s in scenes)


def _q2_deterministic_verdict(scenes: List[Scene], effective_config: dict) -> tuple:
    """Compute deterministic Q2 verdict from pillar markers.

    Returns (verdict: str, note: str).

    Bands (operator-recalibratable):
      targets 25%/50%/75%; within ±7% → PASS; ±12% → WEAK; beyond → FAIL.
      Any of the three twist markers missing → FAIL (deficit).
    """
    total = len(scenes)
    if total == 0:
        return ('FAIL', 'Q2 deterministic: no scenes parsed.')

    targets = {
        'TWIST1': effective_config.get('twist_1_position', 25),
        'TWIST2': effective_config.get('twist_2_position', 50),
        'TWIST3': effective_config.get('twist_3_position', 75),
    }

    # Collect twist positions
    twist_positions = {}
    for s in scenes:
        if s.pillar in _TWIST_PILLARS:
            twist_positions[s.pillar] = s.number

    # Check for missing markers
    missing = [t for t in _TWIST_PILLARS if t not in twist_positions]
    if missing:
        return (
            'FAIL',
            f"Q2 deterministic: missing pillar marker(s): {', '.join(missing)}. "
            f"All three twist markers required when [PILLAR:] markers are present."
        )

    # Compute positions and evaluate bands
    details = []
    worst_verdict = 'PASS'
    for twist_name in _TWIST_PILLARS:
        scene_num = twist_positions[twist_name]
        actual_pct = (scene_num / total) * 100
        target_pct = targets[twist_name]
        delta = abs(actual_pct - target_pct)

        if delta <= _Q2_PASS_BAND:
            band_verdict = 'PASS'
        elif delta <= _Q2_WEAK_BAND:
            band_verdict = 'WEAK'
        else:
            band_verdict = 'FAIL'

        details.append(
            f"{twist_name}: Scene {scene_num}/{total} = {actual_pct:.1f}% "
            f"(target {target_pct}%, delta {delta:.1f}%, {band_verdict})"
        )

        if VERDICT_SEVERITY.get(band_verdict, 0) > VERDICT_SEVERITY.get(worst_verdict, 0):
            worst_verdict = band_verdict

    note = f"Q2 deterministic (pillar markers): {'; '.join(details)}."
    return (worst_verdict, note)
...
# Checks whose verdicts are computed deterministically from raw material,
# not from LLM verdict majority.  Their LLM verdicts are ignored after
# per-scene / word-count data is collected.
# V26: Q2 is added to this set when [PILLAR:] markers are present.
DETERMINISTIC_CHECKS_BASE = {'Q8', 'Q19'}


def _get_deterministic_checks(parsed_scenes: List[Scene]) -> set:
    """Return the set of deterministic check IDs.

    Q2 joins when pillar markers are present in parsed scenes.
    """
    checks = set(DETERMINISTIC_CHECKS_BASE)
    if _q2_has_pillar_markers(parsed_scenes):
        checks.add('Q2')
    return checks


# Back-compat alias (used in _run_single_audit_pass, _merge_multipass_results)
DETERMINISTIC_CHECKS = DETERMINISTIC_CHECKS_BASE
...
def _run_single_audit_pass(
    ...
    # Q19 per-pass deterministic override: apply bands from this pass's
    # per-scene classifications so the per-pass log shows the correct verdict.
    if q19_item and q19_item.get('post_climax_scenes'):
        try:
            _d_count, _d_table = _count_denouement_scenes(q19_item)
            q19_item['verdict'] = _q19_band_verdict(_d_count)
            q19_item['note'] = _q19_band_note(_d_count, _d_table)
        except ValueError:
            pass  # leave LLM verdict if structured data missing
    ...
    # Q8 per-pass deterministic override: word-count bands.
    # Applied here so the per-pass log shows the correct verdict.
    word_min = effective_config['target_synopsis_word_min']
    word_max = effective_config['target_synopsis_word_max']
    for _it in call_2_data.get('items', []):
        if _it.get('id') == 'Q8':
            _it['verdict'] = _q8_band_verdict(word_count, word_min, word_max)
            _it['note'] = _q8_band_note(word_count, word_min, word_max)
            break

    # V26: Q2 per-pass deterministic override when pillar markers present.
    if _q2_has_pillar_markers(parsed_scenes):
        q2_verdict, q2_note = _q2_deterministic_verdict(parsed_scenes, effective_config)
        for _it in call_1_data.get('items', []):
            if _it.get('id') == 'Q2':
                _it['verdict'] = q2_verdict
                _it['note'] = q2_note
                break
...
def _merge_multipass_results(
    ...
    parsed_scenes: List[Scene] = None,
) -> tuple[dict, dict]:
    """Merge N pass results into a single (call_1_data, call_2_data).

    Pipeline order per check type:
    1. Purely LLM-judged (Q1, Q5, Q10, Q12, Q13, Q16, Q17, Q18, and Q2 when
       no pillar markers): majority-vote verdicts across passes.
    2. LLM-judged with deterministic post-processing (Q8, Q19, and Q2 when
       pillar markers present): majority-aggregate RAW MATERIAL only
       (Q19: per-scene classifications; Q8: word count is already deterministic;
       Q2: pillar scene positions) → apply bands → band verdict is FINAL.
    3. Overall verdict computed last from final per-check verdicts.
    """
    ...
    # ── Step 1: majority-vote LLM verdicts for non-deterministic checks ──
    det_checks = _get_deterministic_checks(parsed_scenes or [])
    for items_list in [final_c1.get('items', []), final_c2.get('items', [])]:
        for item in items_list:
            qid = item.get('id')
            if qid and qid in q_verdicts and qid not in det_checks:
    ...
    # ── Step 2a: Q19 — majority-vote per-scene classifications (raw material),
    #             then apply denouement-count bands deterministically ──
        ...
        for items_list in [final_c1.get('items', []), final_c2.get('items', [])]:
            for item in items_list:
                if item.get('id') == 'Q19':
                    ...
                    item['verdict'] = _q19_band_verdict(denouement_count)
                    item['note'] = _q19_band_note(denouement_count, classification_table)

    # ── Step 2b: Q8 — apply word-count bands deterministically ──
    word_min = effective_config['target_synopsis_word_min']
    word_max = effective_config['target_synopsis_word_max']
    for items_list in [final_c1.get('items', []), final_c2.get('items', [])]:
        for item in items_list:
            if item.get('id') == 'Q8':
                item['verdict'] = _q8_band_verdict(word_count, word_min, word_max)
                item['note'] = _q8_band_note(word_count, word_min, word_max)

    # ── Step 2c: Q2 — deterministic when pillar markers present ──
    if parsed_scenes and _q2_has_pillar_markers(parsed_scenes):
        q2_verdict, q2_note = _q2_deterministic_verdict(parsed_scenes, effective_config)
        for items_list in [final_c1.get('items', []), final_c2.get('items', [])]:
            for item in items_list:
                if item.get('id') == 'Q2':
                    item['verdict'] = q2_verdict
                    item['note'] = q2_note
...
def main():
    ...
    call_1_data, call_2_data = _merge_multipass_results(
        all_passes, title, word_count, effective_config, parsed_scenes
    )
